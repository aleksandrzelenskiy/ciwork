...
type OrgPlansPanelProps = {
    orgSlug: string;
    showClose?: boolean;
    onClose?: () => void;
};

export default function OrgPlansPanel({ orgSlug, showClose = false, onClose }: OrgPlansPanelProps) {
    ...
    const pageBg = isDarkMode ? '#0d1117' : '#f5f5f7';
    const surfaceBg = isDarkMode ? 'rgba(21,27,39,0.98)' : '#ffffff';
    const surfaceBorder = isDarkMode ? 'rgba(148,163,184,0.2)' : 'rgba(15,23,42,0.08)';
    const surfaceShadow = isDarkMode ? '0 30px 80px rgba(0,0,0,0.45)' : '0 30px 80px rgba(15,23,42,0.12)';
    const accent = isDarkMode ? '#93c5fd' : '#0f172a';
    const pillButtonSx = {
        borderRadius: 999,
        textTransform: 'none',
        fontWeight: 600,
    ...
    const loadData = React.useCallback(async () => {
        ...
        try {
            ...
            if (!plansRes.ok || !plansPayload || !('plans' in plansPayload)) {
                setError(
                    plansPayload && 'error' in plansPayload
                        ? plansPayload.error ?? 'Не удалось загрузить тарифы'
            ...
            if (!subRes.ok || !subPayload || !('subscription' in subPayload)) {
                setError(
                    subPayload && 'error' in subPayload
                        ? subPayload.error ?? 'Не удалось загрузить подписку'
        ...
        try {
            ...
            if (!res.ok || !payload || !('subscription' in payload)) {
                if (
                    ...
                    setNotice(
                        payload && 'error' in payload ? payload.error ?? 'Не удалось сменить тариф' : 'Не удалось сменить тариф'
                    );
                }
                return;
            }
    ...
    const handleCancelPending = async () => {
        ...
        try {
            ...
            if (!res.ok || !payload || !('subscription' in payload)) {
                setNotice(
                    payload && 'error' in payload ? payload.error ?? 'Не удалось отменить смену тарифа' : 'Не удалось отменить смену тарифа'
                );
                return;
            }
            setSubscription(payload.subscription);
        ...
        >
            <Stack spacing={3.5} sx={{ maxWidth: 1200, mx: 'auto' }}>
                ...
                {subscription?.pendingPlan && subscription?.pendingPlanEffectiveAt && (
                    <Alert
                        ...
                        action={
                            canChangePlan ? (
                                <Button color="inherit" size="small" onClick={handleCancelPending} sx={pillButtonSx}>
                                    Отменить
                                </Button>
                            ) : undefined
                        }
                    >
                        Запланирована смена тарифа на {subscription.pendingPlan.toUpperCase()} с{' '}
                        ...
                        return (
                            <Card
                                ...
                                sx={{
                                    ...planCardSx,
                                    flex: 1,
                                    borderColor: isCurrent ? '#0f172a' : surfaceBorder,
                                }}
                            >
                                <CardHeader
                                    title={plan.title}
                                ...
                                <CardContent>
                                    <Stack spacing={1.5}>
                                        ...
                                        <Typography variant="caption" color="text.secondary">
                                            Сверх лимита: {plan.storageOverageRubPerGbMonth} ₽/GB/мес (помесячно, списание по
                                            часам)
                                        </Typography>
                                        <Divider />
                                        <Stack spacing={0.5}>
                                        ...
                                        {plan.plan === 'enterprise' ? (
                                            <Button variant="outlined" fullWidth disabled sx={pillButtonSx}>
                                                Индивидуально
                                            </Button>
                                        ) : (
                                            <Button
                ...
                <Card variant="outlined" sx={{ ...planCardSx, borderRadius: 3 }}>
                    <CardHeader title="Дополнительное хранилище" titleTypographyProps={{ fontWeight: 700 }} />
                    <CardContent>
                        <Stack spacing={2}>
                            ...
                            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center">
                                <Box>
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        {currentPlanConfig?.storagePackageRubMonthly ?? 0} ₽ / мес (списывается пропорционально
                                        оставшимся часам)
                                    </Typography>
                                </Box>
                                <Button
            ...
            >
                    ...
                    <Box sx={{ mb: 2 }}>
                        {previewLoading ? (
                            ...
                            changeTiming === 'immediate' ? (
                                previewDelta !== null && previewDelta > 0 ? (
                                    ...
                                    <Typography variant="body2">Перерасчет не требуется, сумма к списанию 0 ₽.</Typography>
                                )
                            ) : (
                                <Typography variant="body2">
                        ...
                        {paymentPreview?.willFail && changeTiming === 'immediate' && (
                            <Typography variant="caption" color="error">
                                Недостаточно средств: нужно {formatAmount(paymentPreview.required)} ₽, доступно{' '}
                                {formatAmount(paymentPreview.available)} ₽.
                            </Typography>
                        )}
                    </Box>
                    <RadioGroup
                ...
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button onClick={() => setChangeDialogOpen(false)} sx={pillButtonSx}>
                        Отмена
                    </Button>
                    <Button variant="contained" onClick={confirmChange} disabled={!changeTarget} sx={pillButtonSx}>
                        Подтвердить
                    </Button>
                </DialogActions>
            </Dialog>
