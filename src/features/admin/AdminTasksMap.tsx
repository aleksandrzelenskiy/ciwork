...
const UNASSIGNED_REGION = 'Без региона';
const UNASSIGNED_ORG = 'Без организации';
...
const collectBsNumbers = (task: AdminTaskLocation): string[] => {
...
const buildRelatedNumbers = (task: AdminTaskLocation, pool: string[], currentIndex: number): string | null => {
...
const normalizeExecutorLabel = (task: AdminTaskLocation): string => {
    const name = task.executorName?.trim();
    const email = task.executorEmail?.trim();
    return name || email || UNASSIGNED_LABEL;
};

const normalizeRegionLabel = (task: AdminTaskLocation): string => {
    const region = task.projectRegionCode?.toString().trim();
    return region || UNASSIGNED_REGION;
};

const normalizeOrgLabel = (task: AdminTaskLocation): string => {
    const org = task.orgName?.trim();
    return org || UNASSIGNED_ORG;
};

export default function AdminTasksMap(): React.ReactElement {
    const { t } = useI18n();
    const [tasks, setTasks] = React.useState<AdminTaskLocation[]>([]);
    ...
    React.useEffect(() => {
        let active = true;
        const extractTasks = (payload: unknown): AdminTaskLocation[] => {
            if (!payload || typeof payload !== 'object') return [];
            const value = payload as { tasks?: unknown; items?: unknown };
            if (Array.isArray(value.tasks)) return value.tasks as AdminTaskLocation[];
            if (Array.isArray(value.items)) return value.items as AdminTaskLocation[];
            return [];
        };
        ...
        (async () => {
            ...
            setError(null);
            try {
                const tasksResponse = await fetch(withBasePath('/api/admin/tasks'), { cache: 'no-store' });
                const payload = (await tasksResponse.json().catch(() => null)) as
                    | { tasks?: AdminTaskLocation[]; error?: string }
                    | null;
                if (!active) return;
    ...
    const placemarks = React.useMemo(() => {
        ...
        for (const task of tasks) {
            ...
            task.bsLocation.forEach((loc, idx) => {
                ...
                const executorLabel = normalizeExecutorLabel(task);
                const regionLabel = normalizeRegionLabel(task);
                const orgLabel = normalizeOrgLabel(task);
                ...
                result.push({
                    ...
                    orgName: task.orgName ?? null,
                    orgSlug: task.orgSlug ?? null,
                    orgLabel,
    ...
    React.useEffect(() => {
        setRegionFilter((prev) => {
            ...
            availableRegions.forEach((region) => {
                if (next[region] === undefined) next[region] = true;
            });
            return next;
        });
    }, [availableRegions]);
    ...
    React.useEffect(() => {
        setOrgFilter((prev) => {
            ...
            availableOrgs.forEach((org) => {
                if (next[org] === undefined) next[org] = true;
            });
            return next;
        });
    }, [availableOrgs]);
    ...
    const filteredPlacemarks = React.useMemo(() => {
        ...
        return placemarks.filter((point) => {
            const matchesSearch =
                ...
                (point.executorName ? point.executorName.toLowerCase().includes(query) : false) ||
                (point.executorEmail ? point.executorEmail.toLowerCase().includes(query) : false) ||
                (point.regionCode ? point.regionCode.toLowerCase().includes(query) : false);
    ...
    const buildBalloonContent = React.useCallback((point: MapPoint) => {
        const projectRef = point.projectKey;
        const linkHref =
            point.taskIdentifier && point.orgSlug && projectRef
                ? `/org/${encodeURIComponent(point.orgSlug)}/projects/${encodeURIComponent(
                      projectRef
        ...
        const operatorLine = point.operatorLabel
            ...
            : '';
        const regionLine = point.regionCode
            ? `<div style="margin-bottom:4px;">Регион: ${point.regionCode}</div>`
            : '';
        const managerLine =
        ...
        const executorLine =
            point.executorName || point.executorEmail
                ...
                : `<div style="margin-bottom:4px;">Исполнитель: ${UNASSIGNED_LABEL}</div>`;
    ...
    const operatorLabel = availableOperators.length
        ? `Операторы · ${activeOperatorCount}/${availableOperators.length}`
        : 'Операторы';
            ...
            >
                ...
                {filtersOpen && (
                    ...
                    >
                        <Stack spacing={1.5}>
                            <TextField
                                fullWidth
                                placeholder="Поиск по БС, задаче, проекту или подрядчику"
                                value={search}
                            ...
                            <Stack spacing={1}>
                                ...
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                                    {availableRegions.map((region) => {
                                        const active = regionFilter[region] ?? true;
                                ...
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                                    {availableOrgs.map((org) => {
                                        const active = orgFilter[org] ?? true;
                ...
                {showEmptyState && (
                    <Alert severity="info" sx={{ pointerEvents: 'auto', width: { xs: '92vw', sm: '100%' } }}>
                        Нет задач с координатами в системе.
                    </Alert>
                )}
                {error && (
