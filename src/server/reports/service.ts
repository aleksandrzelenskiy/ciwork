...
type ReportsServiceResult =
    | {
          ...
          userRole: ReturnType<typeof mapRoleToLegacy>;
          isSuperAdmin: boolean;
      }
    | { ok: false; error: string; status: number };

export const listReportsForRequest = async ({
    token,
}: {
    token?: string;
}): Promise<ReportsServiceResult> => {
    ...
    if (guestAccess) {
        ...
        if (activeOrgId && !isSuperAdmin) {
            ...
            const org = await OrganizationModel.findById(activeOrgId)
                .select('orgSlug')
                .lean();
            orgSlug = org?.orgSlug ?? null;
        }
        ...
        normalizedEmail = userContext.data.user.email?.trim().toLowerCase() ?? '';
    }

    let rawReports;
    try {
        rawReports = await ReportModel.find(query).lean();
    } catch (error: unknown) {
    ...
    visibleReports.forEach((report) => {
        ...
        const projectId =
            report.projectId?.toString?.() ?? taskMetaEntry?.projectId ?? null;
        ...
        const canDelete =
            normalizedEmail.length > 0 && !isGuest
                ? managers.some(
                      (manager) =>
                ...
                : false;
        const orgIdValue =
            report.orgId?.toString?.() ?? taskMetaEntry?.orgId ?? null;
